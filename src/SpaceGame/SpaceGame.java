 ...
 */
	public void updateFrame(Graphics2D g){ 
		...
		if(_aliensBulk != null && _wave3 && !_pause){// lol or _start
			if(Math.random()*100 > 95){ // chance of respawning
				Alien2 a  = new Alien2();
				a.setTopX(getWidth() + 10);
				a.setTopY((int)(Math.random()*(getHeight() - 26)));
				a.setSpeedX((int)(Math.random()*2));
		...
		//Removes bullets out of game
		for(int i = 0; _bullets != null && i < _bullets.size(); i++){
			Bullet b = _bullets.get(i);
			if(b.getTopX() > getWidth()){
				_bullets.remove(i);
			}
		}
	...
	*/
	...
	private void drawIntro(Graphics2D g){
		...
		g.drawString("SpaceATTACK" , getWidth()/2 - 150 , getHeight()/2 - 170);
		...
		g.drawString("Move ... W/S  or Up_Key/ Down_key " , getWidth()/2 - 190, getHeight()/2 + 30);
		g.drawString("Pause ... P_Key" , getWidth()/2 - 90, getHeight()/2 + 50);
